<template>
  <div class="relative-position absolute-bottom" style="margin-bottom: 150px">
    <div class="absolute-center" @click="openBuy">
      <img class="cursor-pointer" src="/buy.png" style="width: 150px" />
    </div>

    <!-- DIALOGS -->
    <q-dialog
      v-model="dialog"
      @hide="select_ticket = null"
      maximized
      transition-show="slide-up"
      transition-hide="slide-down"
      transition-duration="600"
    >
      <div class="dialog-mobile no-box-shadow absolute">
        <div class="absolute-right q-mt-xs z-max">
          <q-btn
            round
            icon="close"
        ...
        <q-virtual-scroll
          class="transparent q-py-sm q-pr-sm absolute-center"
        ...
        >
          <tr :key="item">
            <td v-for="tr in item.tr" :key="tr.ruta" class="q-pa-xs">
              <q-img
                :src="tr.ruta"
                style="width: 54px"
...
export default defineComponent({
  ...
  setup() {
    ...
    async function selectEvent() {
      ...
      await event.getTickets($q.platform.is.mobile);
...
.dialog-mobile {
  background-image: url("/dialog-mobile.png");
